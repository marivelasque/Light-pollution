...
```{r raw data check, fig.width=8, fig.height=8, warning=FALSE}
...
```{r results priors, results='asis', eval=TRUE}
...
```{r IIV model, warning = FALSE, eval=TRUE}
...
MR_interact_res %>%
  kable(caption = "Table S11:Results of LMM, the interactive effect of ALAN, time of the day and cross-over design on metabolic rate") %>% 
kable_styling(full_width = T) %>%
  row_spec(MR_interact_res_high, bold = T) %>% 
    footnote(general = "Reference values: Treatment – ALAN; Period – B; Time - PM")
