...
#setwd("/Users/marianavelasque/Dropbox/Pesquisa/Doutorado\ 2/PhD\ Analysis/Experiment\ 2")

load("/Users/marianavelasque/Dropbox/Pesquisa/Doutorado\ 2/PhD\ Analysis/Experiment\ 2/MCMC_results_light_polution.RData")

#save.image("/Users/marianavelasque/Dropbox/Pesquisa/Doutorado\ 2/PhD\ Analysis/Experiment\ 2/MCMC_results_light_polution.RData")
...
```{r dall data, eval=FALSE, warning=FALSE, include=TRUE}
lightdark=read_csv("data/light_pollution_val_table.csv", 
                     col_types = cols()) %>% 
  mutate(id, factor(id), srtime= factor(srtime), treatment = factor(treatment), 
         time_treatment = paste0(srtime, "_", treatment)) %>% 
  as_tibble() 
...
```{r raw data check, fig.height=8, fig.width=8, warning=FALSE}
...
The effect of Constant light (a) and the interaction (b) between treatment and time (day or night) on the startle response duration of the hermit crab *Pagurus bernhardus*. We used hierarchical generalized linear models (HGLM) to predict the effect of Constant light and time on which the startle response was induced to predict its duration. 

<br><br>

# 1. The effect of Constant light on the startle response duration
...
The effect of Constant light on the startle response duration of hermit crabs
```{r plot mean effect for sr, eval=FALSE, fig.height=4, fig.width=8, message=FALSE, warning=FALSE, include=TRUE}
...
g1<- lightdark %>% 
   ...
   labs(x = "\n Light condition", y = "Startle response duration (predicted)") +
   scale_fill_manual(values=c("orange", "#3c6a8f"), labels = c("Day", "Night"), name = "Time") + 
   #guides(fill=FALSE)  +
   scale_x_discrete(labels = c("Standard", "Constant")) + 
...
g2<- lightdark %>% 
   ...
   geom_boxplot(size = 0.75, width = 0.25) + labs(x = "\n Light condition", 
...
ggsave("g1.png",
        units="in", width=8, height= 4, dpi=600)
```
<br>

<br> _Figure S2._ The effect of Constant light (a) and the interaction (b) between treatment and time (day or night) on the startle response duration of the hermit crab *Pagurus bernhardus*. We used the HGLM model to predict the startle response duration. 
...
IIV_matrix<-matrix(c("VID_Day_LD",0,0,0,
                     0,"VID_Night_LD",0,0,
                     0,0,"VID_Day_Constant light",0,
                     0,0,0,"VID_Night_Constant light"),
...
We began by estimating the posterior modes for the repeatabilities across experimental blocks (i.e. Constant light during day and night, standard light regime during day and night). 
...
#Repeatability of day measures in the Constant light treatment
...
#Repeatability of night measures in the Constant light treatment

VID_Night_PL = M2.2$VCV[,4]; VR_Night_PL = M2.2$VCV[,8]
R_Night_PL = VID_Night_PL/(VID_Night_PL + VR_Night_PL)

#Difference in repeatability between day and night in the Constant light treatment
...
#Difference in repeatability between Constant light and the light and dark treatment during day
...
deltas<-  paste0("\u0394", c(" Constant light", " Standard light", " Day", " Night"))

Repeat<-tibble(
   name = c("Day - Standard light", "Night - Standard light", "Day - Constant light", "Night - Constant light",deltas),
...
Repeat1 <-  Repeat %>% 
  mutate(name = str_replace(name, "LD", "- Standard light"),
         name = str_replace(name, "PL", "- Constant light"),
         name = str_replace(name, "Δ ", "Δ - "),
         name = str_replace(name, "AM", "- AM"),
         name = str_replace(name, "PM", "- PM"))


plt1 <- ggplot(Repeat1, aes(x = name, y = mean, ymin = upper, ymax = lower)) +
...
plt1 +  scale_color_manual(values=c("orange", "orange","#172c3d", "orange",
   ...
   theme(legend.position = "none") 
```
<br> _Figure S3._ Forest plot for the repeatability estimates between treatment groups within periods and ∆R of differences between treatments (∆R = Constant light - Standard light) and between the time on which the startle response was induced ∆R (Night-Day). Repeatability and difference in repeatability (∆) values were considered significant when 95% CIs of their posterior modes did not overlap zero.
...
htmlTable(as.data.frame(Repeat1[,-1]), 
          header =   paste(c("Posterior mean","95% CI lower","95% CI upper")),
          rnames = paste(c(Repeat1$name)),
          rgroup = c(repeat_L),
          n.rgroup = c(4, 4), 
          #          col.rgroup = c(rows=c(AD1),"#c95400"),
          #          col.rgroup = c(rows=c(AD2),"#c95400"),
          ...
          tfoot= "Differences between treatments and time (∆R) was estimated as Constant light - Standard light and was Day - Night")
```
...
#Between-individual variation during Constant light treatment morning versus night 
...
plt2 <- ggplot(VID1, aes(x = name, y = mean, ymin = upper, ymax = lower)) +
   geom_linerange(aes(color =  name), size = 1, alpha = 0.5) +
   geom_point(aes(color = name), 
              position=position_dodge(width=c(0.6,0.4)), size = 3) +
   coord_flip() + 
...
```
<br> _Figure S4._ Forest plot for between-individual variation between treatment groups within periods and ∆R of differences between treatments (∆R = Constant light - Standard light) and between the time on which the startle response was induced ∆R (Night-Day). Between-individual variation and ∆ values were considered significant when 95% CIs of their posterior modes did not overlap zero.
<br>
```{r table VID}
VID1 <- VID1 %>% 
   mutate_at(c( "mean","lower", "upper"),as.numeric) %>% 
...
repeat_VID1<- c("Repeatability estimate", paste0("\u0394", " Variance"))

htmlTable(as.data.frame(VID1[,-1]), 
          header =   paste(c("Posterior mean","95% CI lower","95% CI upper")),
          rnames = paste(c(Repeat1$name)),
          rgroup = c(repeat_VID1),
          n.rgroup = c(4, 4), 
          #          col.rgroup = c(rows=c(AD1),"#c95400"),
          #          col.rgroup = c(rows=c(AD2),"#c95400"),
          ...
          tfoot= "Differences between treatments and time (∆V) was estimated as Constant light - Standard light and was Day - Night")
...
<br>
```{r WID, eval=FALSE,  warning=FALSE, include=TRUE}
...
plt3 <- ggplot(WID1, aes(x = name, y = mean, ymin = upper, ymax = lower)) +
   geom_linerange(aes(color =  name), size = 1, alpha = 0.5) +
...
```
<br> _Figure S4._ Forest plot for within-individual variation between treatment groups within periods and ∆R of differences between treatments (∆R = Constant light - Standard light) and between the time on which the startle response was induced ∆R (Night-Day). Within-individual variation and ∆ values were considered significant when 95% CIs of their posterior modes did not overlap zero.
<br>
```{r table WID1}
WID1 <- WID1 %>% 
   mutate(name = str_replace(name, "ALAN", "Constant light")) %>% 
   mutate(name = str_replace(name, "standard", "Standard")) %>% 
   mutate_at(c( "mean","lower", "upper"),as.numeric) %>% 
   mutate_at(vars("mean","lower", "upper"), funs(round(., 3))) %>% 
   select("name", "mean","lower", "upper")

repeat_WID1<- c("Repeatability estimate", paste0("\u0394", " Variance"))

htmlTable(as.data.frame(WID1[,-1]), 
          header =   paste(c("Posterior mean","95% CI lower","95% CI upper")),
          rnames = paste(c(WID1$name)),
          rgroup = c(repeat_VID),
          n.rgroup = c(4, 4), 
          #          col.rgroup = c(rows=c(AD1),"#c95400"),
          #          col.rgroup = c(rows=c(AD2),"#c95400"),
...
# 3. The effect of Constant light on metabolic rate

<br>
To investigate the effect of light pollution on the behaviour of hermit crabs, a linear Linear mixed-effects model fit by REML. Metabolic rate was log-transformed (log~10~ +1). To account for the cross-over effect, we built a model containing a random intercept (i.e. individuals) and  random slopes (i.e. experimental period), indicating that individual slopes vary across different experimental periods).

<br>

```{r comp_mr, message=FALSE, warning=FALSE}
MR <- read_csv("data/MR_res.csv", col_types = cols(ID = col_character(), 
     Period = col_character(), Time = col_character())) %>% 
  mutate(metR = log10(`metabolic rate` +1 )) %>% 
  arrange(desc(Treatment))


Period_int <- lmer(metR  ~ Treatment*Period*Treatment*Time +
                     (Period|ID),
                     data=MR)


MR_interact_res<- summary(Period_int)

MR_interact_res = MR_interact_res$coefficients %>% 
  data.frame() %>% 
  rownames_to_column("headings") %>% 
  rename(`Pr(>|z|)` = `Pr...t..`) %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  mutate(`Pr(>|z|)` = as.character(`Pr(>|z|)`)) %>% 
  mutate(`Pr(>|z|)` = case_when(
    `Pr(>|z|)` < 0.0001 ~ "< 0.001",
     TRUE ~ `Pr(>|z|)`)) %>% 
  mutate(headings = str_remove(headings, "2"),
         headings = str_remove(headings, "2"),
         headings = str_remove(headings, "LL")) %>% 
  column_to_rownames("headings") %>% 
  rename(Std.Error = Std..Error)




MR_interact_res_high <- which(MR_interact_res$`Pr(>|z|)` < 0.05 |
                        MR_interact_res$`Pr(>|z|)` == "< 0.001" )

MR_interact_res %>%
  kable(caption = "Table S11:Results of LMM, the interactive effect of Constant light, time of the day and cross-over design on metabolic rate") %>% 
kable_styling(full_width = T) %>%
  row_spec(MR_interact_res_high, bold = T) %>% 
    footnote(general = "Reference values: Treatment – Constant light; Period – B; Time - PM")

```

<br><br>

As the MCMCglmm model did not achieve convergence when we specified an interaction between treatment group and haemocyanin, we test its effect independently. We used t-test as haemocyanin concentration was performed in a single animal at the end of the experiment. 

```{r t_testhae}
lightdark_tst <- lightdark %>% 
  group_by(treatment, obs) %>%
  summarise(haemoporc = mean(haemoporc)) 

haemo_test <- t.test(log(haemoporc) ~ treatment, 
                     data = lightdark_tst, var.equal = FALSE,
                      alternative = c("two.sided")) %>% 
  tidy() %>% 
  dplyr::select(t= statistic, df = parameter, p.value ) %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  as.data.frame()

rownames(haemo_test) = "Haemocyanin concentration"

havemo_h <- which(haemo_test$`p.value` < 0.05 |
                        haemo_test$`p.value` == "< 0.001" )

haemo_test  %>%
  kable(caption = "Table S12: T-test results for the effect of treatent on Haemocyanin concentration") %>% 
kable_styling(full_width = T) %>%
  row_spec(havemo_h, bold = T)
  
```

```{r plot_data_mr, eval=FALSE, fig.height=9, fig.width=12, include=T}
Mt<- MR %>% 
   group_by(Treatment) %>% 
   ggplot(aes(y =log10(metR +1), x = Treatment, fill =Treatment)) +
   geom_boxplot(size = 0.8, width = 0.4) + 
   labs(x = "\n Light condition", y = "Metabolic rate \n (log10 nmol O2 mg−1 h−1 STP)") +
...
g1<- lightdark %>% 
   ...
   ggplot(aes(x =treatment, y = prediction, fill =srtime)) +
   geom_boxplot(size = 0.8, width = 0.4) + 
   labs(x = "\n Light condition", y = "Starle response duration (s)") +
   scale_fill_manual(values=c("orange", "#3c6a8f"),
                     labels = c("Day", "Night"), name = "") + 
   #guides(fill=FALSE)  +
   #scale_x_discrete(labels = c("Standard light", "Constant light")) + 
   theme_classic(base_size = 12) + 
...
```
<img width="100%" src="/Users/marianavelasque/Dropbox/Pesquisa/Doutorado 2/PhD Analysis/Experiment 2/g2.png"/>

_Fig S5._ The effect of Constant light on (A) boldness,  the (B) metabolic rate (MO2) and (C) haemocyanin concentration (mMol 1<sup>-1</sup>) in *Pagurus bernhardus*. Metabolic rate is expressed as log<sup>10</sup> nmol O<sub>2</sub> mg<sup>−1</sup> h<sup>−1 STP</sup> 
